...
if ($testExitCode -eq 0) {
    # Parse test results
    $testOutput -match "Passed!\s+-\s+Failed:\s+(\d+),\s+Passed:\s+(\d+),\s+Skipped:\s+(\d+),\s+Total:\s+(\d+)" | Out-Null
    $passed = $matches[2]
    $total = $matches[4]
...
if ($buildExitCode -eq 0) {
    ...
    Write-Host ""
    Write-Host "1️⃣  Start the API:" -ForegroundColor White
    Write-Host "    cd src\Api" -ForegroundColor Gray
    Write-Host "    dotnet run" -ForegroundColor Gray
    Write-Host ""
    
    Write-Host "2️⃣  Test workflow endpoint (in new terminal):" -ForegroundColor White
    Write-Host '    Invoke-RestMethod -Uri "http://localhost:5195/api/workflow/test-workflow" -Method Post' -ForegroundColor Gray
    Write-Host ""
    
    Write-Host "3️⃣  Check generated output:" -ForegroundColor White
    Write-Host "    explorer C:\Temp\Documentation-Catalog\temp" -ForegroundColor Gray
    Write-Host ""
    
    Write-Host "4️⃣  View Swagger UI:" -ForegroundColor White
    Write-Host "    http://localhost:5195/swagger" -ForegroundColor Gray
    Write-Host ""
    
    Write-Host "5️⃣  Optional: Apply remaining fixes from COMPLETE_FIX_PACKAGE.md" -ForegroundColor White
