...
public class DocumentBusinessRules
{
    ...
    public static bool CanTransitionToStatus(DocumentStatus currentStatus, DocumentStatus targetStatus)
    {
        if (currentStatus == DocumentStatus.Draft)
            return targetStatus == DocumentStatus.UnderReview;
        
        if (currentStatus == DocumentStatus.UnderReview)
            return targetStatus == DocumentStatus.Published || targetStatus == DocumentStatus.Draft;
        
        if (currentStatus == DocumentStatus.Published)
            return targetStatus == DocumentStatus.Archived;
        
        if (currentStatus == DocumentStatus.Archived)
            return false; // Archived documents cannot transition
